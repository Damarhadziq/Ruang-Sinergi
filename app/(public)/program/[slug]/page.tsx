...
import { ProgressTrack } from "@/components/gamification/progress-track";
import { Milestone } from "@/components/gamification/milestone";
...
export default async function ProgramPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const completedCount = Math.min(items.length, Math.floor(program.progress / 34));
  const currentMaterial = items[completedCount] ?? items.at(-1);
    ...
    <main>
      <section className="program-hero">
        <div className="mx-auto grid w-full max-w-[1180px] gap-8 px-4 py-12 sm:px-6 sm:py-16 lg:grid-cols-[minmax(0,1fr)_410px] lg:px-8">
          <FadeIn className="self-center">
            <DetailBackLink href="/program" label="Kembali ke daftar program" className="mb-8" />
      ...
      <section className="mx-auto grid w-full max-w-[1180px] gap-10 px-4 py-14 sm:px-6 sm:py-16 lg:grid-cols-[minmax(0,1fr)_300px] lg:px-8">
        <div className="min-w-0">
          <h2 className="dashboard-title">Materi dalam program ini</h2>
          <ol className="mission-path mt-8">
            {items.map((material, index) => material && (() => {
              const isCompleted = index < completedCount;
              const isCurrent = index === completedCount;
              const stateLabel = isCompleted ? "Selesai" : isCurrent ? "Sedang dipelajari" : "Berikutnya";
              return (
              <li key={material.slug} className="mission-step">
                <div className={`mission-marker${isCurrent ? " is-current" : ""}`}>{isCompleted ? <CheckCircle size={19} weight="BoldDuotone" /> : index + 1}</div>
                <Link href={`/materi/${material.slug}?program=${program.slug}`} className="mission-step-card group">
                  ...
                  <div className="min-w-0 p-5 pr-12">
                    <div className="flex flex-wrap items-center gap-2"><Badge className="bg-[var(--secondary)] text-[var(--primary)]">Tahap {index + 1}</Badge><Milestone state={isCompleted ? "completed" : isCurrent ? "current" : "next"} label={stateLabel} /><span className="text-meta text-[var(--muted-foreground)]">{getDepartment(material.department)?.shortName}</span></div>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="program-progress-panel">
            <h2 className="text-subtitle text-[#315eaf]">Progres program</h2>
            ...
            <div className="mt-4"><ProgressTrack value={program.progress} milestones={items.length} label={`Progres program ${program.progress}%`} /></div>
            <p className="text-small mt-4 text-[var(--muted-foreground)]">{completedCount} dari {items.length} tahap selesai. Lanjutkan kapan saja dari tahap terakhir.</p>
            {currentMaterial && <Link href={`/materi/${currentMaterial.slug}?program=${program.slug}`} className="flex justify-end"><Button className="mt-5 w-fit">Lanjutkan belajar <DirectionArrowRight size={16} /></Button></Link>}
          </div>

          <div className="mt-7 grid gap-5">
            <ProgramBenefit icon={BookBookmark} title="Terstruktur" text="Belajar secara berurutan." />
            <ProgramBenefit icon={PlayCircle} title="Lintas bidang" text="Setiap tahap berasal dari bidang berbeda." />
