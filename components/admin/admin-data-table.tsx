...
function RowActions({ slug }: { slug: string }) {
  ...
  return <div ref={root} className="relative"><Button variant="ghost" size="icon" aria-label="Buka menu aksi" title="Menu aksi" aria-expanded={open} onClick={() => setOpen(!open)}><MoreHorizontal size={17} /></Button><div data-state={open ? "open" : "closed"} aria-hidden={!open} className="motion-popover absolute right-0 z-20 mt-1 w-36 rounded-lg border border-[var(--border)] bg-white p-1"><Link tabIndex={open ? 0 : -1} href={`/materi/${slug}`} className="flex min-h-7 items-center gap-2 rounded-md px-2.5 py-1 text-[11px] transition-colors hover:bg-[#f5f7f5]"><Pencil size={15} />Lihat materi</Link><button tabIndex={open ? 0 : -1} className="flex min-h-7 w-full items-center gap-2 rounded-md px-2.5 py-1 text-[11px] text-[var(--destructive)] transition-colors hover:bg-[#f7eae8]"><Trash2 size={15} />Hapus materi</button></div></div>;
}
