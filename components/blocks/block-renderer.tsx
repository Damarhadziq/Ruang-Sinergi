...
export function BlockRenderer({ blocks }: { blocks: ContentBlock[] }) {
  return <div className="grid gap-6">{blocks.map((block) => {
    switch (block.block_type) {
      case "text": return <section key={block.id} className="prose-copy"><h2>{block.heading}</h2><p>{block.body}</p></section>;
      ...
      case "quiz": return <QuizBlock key={block.id} {...block} />;
      case "reflection": return <ReflectionBlock key={block.id} {...block} />;
      case "code": return <CodeBlock key={block.id} {...block} />;
      case "flowchart": return <div key={block.id} className="block-surface"><h3 className="block-title">{block.title}</h3><div className="mt-5 flex flex-wrap items-center justify-center gap-2">{block.nodes.map((node, i) => <div key={node} className="flex items-center gap-2"><span className="rounded-xl border border-[#ccd9d2] bg-[#f3f8f5] px-4 py-2 text-sm font-semibold text-[#315c4c]">{node}</span>{i < block.nodes.length - 1 && <span className="text-[#8da297]">→</span>}</div>)}</div></div>;
      ...
      case "diagram": return <div key={block.id} className="block-surface"><div className="flex items-center gap-2 text-[#8a741f]"><ShieldCheck size={20} /><h3 className="block-title">{block.title}</h3></div><div className="mt-4 grid min-h-40 place-items-center rounded-xl border border-dashed border-[#d8c982] bg-[#fffdf3] p-6 text-center text-sm leading-6 text-[#70662f]">{block.description}<br /><span className="mt-3 inline-block rounded-lg border bg-white px-3 py-2">Baterai → Sakelar → Lampu → Baterai</span></div></div>;
